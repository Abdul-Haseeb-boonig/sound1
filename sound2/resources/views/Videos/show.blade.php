...
@section('content')
    <div style="background: white; border-radius: 10px; padding: 2rem; box-shadow: 0 3px 15px rgba(0,0,0,0.1); margin-bottom: 2rem;">
        <div style="display: grid; grid-template-columns: 1fr 2fr; gap: 2rem; margin-bottom: 2rem;">
            ...
            <div>
                <h1 style="color: #2c3e50; margin: 0 0 1rem 0; font-size: 2rem;">{{ $video->title }}</h1>
                <p style="font-size: 1.1rem; line-height: 1.6; color: #555; margin-bottom: 1.5rem;">{{ $video->description }}</p>
                
                <div style="display: grid; grid-template-columns: repeat(2, 1fr); gap: 1rem; margin-bottom: 2rem;">
                    <div style="background: #f8f9fa; padding: 1rem; border-radius: 8px;">
                        <strong style="color: #2c3e50;">Artist:</strong>
                        <p style="margin: 0.5rem 0 0 0; color: #666;">{{ $video->artist }}</p>
                    </div>
                    <div style="background: #f8f9fa; padding: 1rem; border-radius: 8px;">
                        <strong style="color: #2c3e50;">Album:</strong>
                        <p style="margin: 0.5rem 0 0 0; color: #666;">{{ $video->album }}</p>
                    </div>
                    <div style="background: #f8f9fa; padding: 1rem; border-radius: 8px;">
                        <strong style="color: #2c3e50;">Year:</strong>
                        <p style="margin: 0.5rem 0 0 0; color: #666;">{{ $video->year }}</p>
                    </div>
                    <div style="background: #f8f9fa; padding: 1rem; border-radius: 8px;">
                        <strong style="color: #2c3e50;">Genre:</strong>
                        <p style="margin: 0.5rem 0 0 0; color: #666;">{{ $video->genre }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div style="background: #2c3e50; padding: 2rem; border-radius: 10px; margin: 2rem 0;">
            <h3 style="color: white; margin: 0 0 1rem 0; text-align: center;">Now Playing: {{ $video->title }}</h3>
            ...
            <div style="text-align: center; margin-top: 1rem;">
                <a href="{{ asset('storage/' . $video->file_path) }}" download 
                   style="background: #3498db; color: white; padding: 0.75rem 1.5rem; text-decoration: none; border-radius: 5px; display: inline-block;">
                    Download Video
                </a>
            </div>
        </div>
    ...
    <div style="background: white; border-radius: 10px; padding: 2rem; box-shadow: 0 3px 15px rgba(0,0,0,0.1);">
        <h2 style="color: #2c3e50; border-bottom: 2px solid #e74c3c; padding-bottom: 0.5rem; margin-bottom: 1.5rem;">Reviews & Ratings</h2>
        
        <div style="background: #f8f9fa; padding: 1.5rem; border-radius: 8px; margin-bottom: 2rem;">
            <h3 style="color: #2c3e50; margin: 0 0 1rem 0;">Average Rating: 
                <span style="color: #f39c12;">{{ number_format($video->averageRating(), 1) }}/5</span>
            </h3>
        </div>
